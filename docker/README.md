...
hf download deepseek-ai/DeepSeek-OCR --local-dir models/deepseek-ai/DeepSeek-OCR
...
Login to AWS CDK:
```bash
# 1) ECR login (must be done once per session/region)
aws ecr get-login-password --region us-east-1 \
| docker login --username AWS --password-stdin 039885961427.dkr.ecr.us-east-1.amazonaws.com

# 2) (once) make sure the repo exists
aws ecr describe-repositories --repository-names deepseek-ocr --region us-east-1 >/dev/null 2>&1 \
